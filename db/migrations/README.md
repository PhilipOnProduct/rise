...
| 0009_rls_lockdown.sql | PHI-61 | Enables RLS on `travelers`, `itineraries`, `travel_connectors` with `auth.uid()`-scoped policies. Admin/system tables stay RLS-off and are accessed via the service-role client (`lib/supabase-admin.ts`). Requires `SUPABASE_SERVICE_ROLE_KEY` in env post-deploy. |
