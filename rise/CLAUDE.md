...
- **Product team tab** — Multi-agent discussion: Sarah (PM) frames the problem, Alex/Maya/Luca/Elena respond in parallel (each at 1024 tokens), Sarah synthesises (2048 tokens). Generates PRD (6000 tokens). If launched from a Kanban card (`pendingObjective`), the generated PRD is saved back to that card's `objectives` row. Sarah's memory persisted in `agent_memory` table. Saves to `team_conversations` (type=`"team"`). PRD generation prompt instructs Claude to include a self-contained Claude Code Implementation Prompt section (under 200 words, no file paths or code snippets).
