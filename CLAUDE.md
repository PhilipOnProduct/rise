...
For persona-based discussion (Sarah PM, Maya Designer, Luca Tech Lead, Elena Travel Expert), see `TEAM.md`. When Philip names a teammate ("ask Maya…", "what would Luca say?") or says "the team", respond in-character per that file. **In Cowork**, invoke the matching `product-management:*` / `design:*` / `engineering:*` plugin skill when the role is active and the situation calls for it — TEAM.md's "When to fire" line on each role spells out the triggers. **In Claude Code (terminal)**, those skills aren't installed; carry the personas and POVs but skip the skill invocations.

---

## Linear issue workflow

Issue tracking lives in Linear (team `Philip On Product`). **`KANBAN.md` is the canonical workflow** — column meanings, label conventions, PRD requirements, handoff steps, archetype testing, regression handling, and every gotcha incident (PHI-51, PHI-74, PHI-79). Read it before picking up or closing an issue. The bullets below are only the shape, not the ritual.

**State transitions are split between Claude Code (terminal) and Cowork (this session).** Claude Code owns the **In Progress** transition on pickup and ships the diff to production. **Cowork owns the Done transition** after running Elena's archetype walks via the Chrome MCP. See KANBAN.md § Archetype testing for the mapping and walk recipe.

- **On pickup (Claude Code)** — first action, before grepping or writing any code: `save_issue` to **In Progress**. Idempotent.
- **On ship (Claude Code)** — after `npm run build` clean + `git push origin main` + Vercel bundle fingerprinted + closing comment filed: `save_issue` to **In Review** (not Done). The In Review status *is* the handoff signal — Cowork queries `list_issues state="In Review"` to find ready cards. See KANBAN.md § Handing off to Claude Code for the full ritual (commit format, fingerprint mechanics, what goes in the comment, deferral rules).
- **On close (Cowork)** — run the archetype walks per KANBAN.md § Archetype testing from `/`, file a separate comment with one paragraph per archetype walked, move to **Done**. Regression → move back to **In Progress** with a failure comment. Unrelated finding → file a new Linear issue, finish this card to Done normally.
- **If blocked (Claude Code)** — `save_comment` with what was tried, leave the card In Progress (don't move backwards). Philip decides whether to bounce it back, scope it down, or split it.
...
### Traveller flows
- **Onboarding wizard** (`/welcome`) — 7-step flow: Step 0 full-screen landing (destination) → Step 1 destination + dates → Step 2 hotel (optional — Places autocomplete biased to destination; "I haven't booked yet — skip →" link skips with null hotel; PHI-100: a second affordance "Don't know yet — help me pick a neighbourhood →" opens the soft picker, calling `/api/neighborhoods` (Claude Haiku, cached in `destination_neighborhoods` keyed case-insensitively) and replacing the hotel input with 4–6 neighbourhood cards (name + honest one-sentence trade-off blurb + best-fit line). Selecting a card sets `anchorNeighborhood` and continues to Step 3; clears any half-typed hotel; "← Back to hotel search" returns. No Anthropic call fires on step-2 mount — only on click. Continue always enabled) → Step 3 travel preferences (company + traveler count + children's ages + style tags + budget tier) → Step 4 (PHI-90) "Anything you already want to do?" — optional free-text must-dos, one per line, fully skippable; the textarea is parsed via `splitSeededActivities()` (trim, drop blanks and >200-char paste accidents, cap at 20 items) and persisted to `travelers.user_seeded_activities` via PATCH on advance and on finish, plus `localStorage.rise_traveler.userSeededActivities`. The list is passed to `/api/itinerary/generate` as anchors — Claude places each on exactly one day in one time block, marks the matching item `seededByUser: true`, and returns a top-level `placement_notes` string when an anchor was filtered out (wrong-city) or couldn't be fitted. PHI-114: `placement_notes`' scope is narrowed to anchor surfacing only — travel facts (closures, pre-booking warnings, seasonal cutoffs, peak-time advice, transport quirks) move to a separate top-level `time_sensitive_alerts: string[] | null` field (max 4, plain travel-planner voice, null when nothing actionable). Both the welcome preview and `/itinerary` render `time_sensitive_alerts` as a "Before you go" amber block ABOVE the "A note on your must-dos:" `placement_notes` callout (`localStorage.rise_itinerary_placement_notes` + `localStorage.rise_itinerary_time_sensitive_alerts`). The prompt also forbids both fields from carrying `REJECTED`, `placed verbatim`, `ANCHOR PLACEMENTS`, or any rejection audit of activities filtered by user preferences. The new step never blocks forward progress: Continue is enabled with an empty textarea, and there's a "Nothing yet — skip →" link below the textarea. → Step 5 AI activity preview (streaming, personalised using Step 3 preferences) → Step 6 account creation (PHI-59: clicking "Send magic link →" PATCHes name + email onto the existing traveler row, saves the local snapshot, calls `supabase.auth.signInWithOtp({email, emailRedirectTo: /auth/callback?next=/dashboard&travelerId=<id>})`, and routes to `/auth/check-email`. The callback links `travelers.auth_user_id` once the user clicks the email. If the OTP send fails, we fall back to `/itinerary` so the user still sees their trip. Name/email inputs have explicit `name` and `autoComplete` attributes to prevent browser autofill cross-contamination. PHI-74: when the user is already signed in (Supabase session present, `authedUser` non-null), the handler PATCHes name+email and saves the local snapshot as before but then routes to `/auth/claim?next=/dashboard` instead of sending a magic link or pushing straight to `/dashboard` — the PHI-60 conflict UI reconciles the new trip against any existing primary trip on the account, and the claim API owns linking `auth_user_id` as part of the chosen action. `localStorage.rise_traveler` is preserved across the hand-off because `/auth/claim` reads it). Preferences are written to Supabase via partial upsert when the user advances from Step 3 to Step 4. Saves to Supabase `travelers` table and `localStorage` (`rise_traveler`, `rise_onboarded`). Step 3 has four sections: (1) "Who's coming?" — two side-by-side steppers for Adults (default 2, min 1) and Children (default 0, min 0); when children > 0, age range rows appear below (each child gets "Child N" label + four selectable buttons: Under 2, 2–4, 5–8, 9–12). (2) "Trip type" — dynamic company selector derived from composition: 1 adult + no children → auto-set "solo" (hidden); 2 adults + no children → show Couple / Friend group; 3+ adults + no children → show Friend group / Family; any children → auto-set "family" (hidden). Label hidden when only one option or auto-set. Invalid selection cleared on composition change. (3) Travel style tags — personalised by group type: universal core (Cultural, Food-led, Relaxed, Adventure, Off the beaten track, History) plus group-specific tags (e.g. Romantic for couples, Kid-friendly/Beach/Educational for families, Nightlife/Active/Festivals for friends, Budget-savvy/Slow travel for solo). Tags cleared on company change if no longer available. (4) Budget tier. Children input is available for any trip type. State uses `adultCount` and `childrenAges` array; `travelerCount` is computed as `adultCount + childrenAges.length` at save time.
...
- **Eval suites** (`/admin/evals/suites`, PHI-119) — In-browser workbench for the `npm run eval:*` suites. Suite picker renders all 10 evals as cards (title, kind: offline/Anthropic-API/needs-dev-server, cost-per-run estimate, case count, last-run timestamp, status dot). Selecting a suite reveals two tabs: **Run** and **History**. Only `eval:family` is wired end-to-end in this card — clicking Run POSTs `/api/admin/evals/suites/family/runs`, which executes `runFamilySuiteForGui()` from `lib/evals/registry.ts` (mirrors the CLI's per-scenario loop, returns structured per-case data — keeps the CLI runner byte-identical per PHI-118 constraint), persists one `eval_suite_runs` row + N `eval_case_runs` rows, and returns the result inline (~200ms total). Run tab shows per-case pass/fail pills with failed-assertion labels for any failures. History tab lists prior runs (most recent first, cap 50) with status dot, pass-rate, cost; clicking a row expands to fetch `/api/admin/evals/suites/<slug>/runs/<runId>` and show per-case detail. Placeholder suites (9 of 10) show "Not yet wired in the GUI" with the matching `npm run eval:<slug>` command. Cards 3–5 of PHI-117 extend this surface — card 3 adds the cost-confirm UI + queued state for paid suites, card 4 wires the heavy multi-run trio, card 5 ships Compare with variance bands. The `SUITES` registry in `lib/evals/registry.ts` is the single source of truth for what shows up in the picker; cards 3+ flip `wired:true` per suite as they wire in.
...
- **Markdown rendering** — Agent, coach, and PM chat bubbles use the `MarkdownText` component (defined in `team/shared.tsx`) to render markdown: `##`/`###` as bold headings, `**text**` as bold, `---` as horizontal rules, `-`/`*` as bullet lists, numbered lists. Kanban card titles and admin discussion titles strip `**`/`*` via `.replace(/\*+/g, "")`.
...
- **Supabase clients** — three flavours, distinct call sites (the anon-keyed legacy client `lib/supabase.ts` was **deleted** in the 0020 RLS lockdown — every public table now has RLS, so the anon key can't touch anything without a policy):
...
- **RLS policies** (PHI-61, `db/migrations/0009_rls_lockdown.sql`) — RLS is enabled on `travelers`, `itineraries`, and `travel_connectors`. Policies are scoped to `auth.uid()`: a user can read/write only the `travelers` row(s) where `auth_user_id = auth.uid()`, and child rows on `itineraries` / `travel_connectors` are reachable only via owned `traveler_id`. Pre-signup welcome rows (`auth_user_id IS NULL`) are invisible to RLS — routes that legitimately serve them (`POST /api/travelers`, `PATCH /api/travelers`, `/api/travelers/claim`, `/api/anonymous-session`) use the service-role admin client and verify ownership in code. Admin/system tables (`ai_logs`, `api_usage`, `api_limits`, `eval_*`, `team_*`, `agent_memory`, `prd_feedback`, `objectives`, `activity_feedback`, `user_feedback`, `profiles`, `anonymous_sessions`, `itinerary_items`, `guides`, `tips`, `tip_ratings`, `destination_neighborhoods`, `popular_picks_cache`) have RLS **enabled with no policies** since the 0020 lockdown (`db/migrations/0020_rls_enable_system_tables.sql`, triggered by the 2026-07-12 Supabase security advisor): the anon and authenticated roles are fully denied (grants revoked too), and every read/write goes through the service-role admin client behind the site_password / ADMIN_PASSWORD perimeter. **Hard constraint (PHI-67, now schema-enforced):** any API route that reads or writes one of these tables MUST use `getSupabaseAdminClient()` — an anon-key call now fails loudly instead of silently bypassing. The former grandfathered callers were migrated in the 0020 change: the guide/tip routes use `lib/guides-server.ts` + the admin client, and the `/team` browser code goes through the `/api/team/*` routes.
...
- **Admin endpoints** (PHI-61) — admin pages (`/admin`, `/admin/usage`, `/admin/evals`, `/admin/evals/suites`) no longer talk to Supabase from the browser. They go through admin-gated API routes that wrap the service-role client server-side: `/api/admin/team-discussions`, `/api/admin/usage-log`, `/api/usage/limits` (GET added), `/api/admin/evals/cases`, `/api/admin/evals/results`, and the PHI-119 suite trio (`/api/admin/evals/suites`, `/api/admin/evals/suites/[suite]/runs`, `/api/admin/evals/suites/[suite]/runs/[runId]`). The team kanban (`/team`) goes through `/api/team/{conversations,objectives,memory,prd-feedback}` (admin client behind the site-password perimeter) since the 0020 RLS lockdown — `app/team/team-data.ts` keeps the same helper signatures but fetches those routes instead of calling Supabase from the browser.
...
- **Eval scripts** — `npm run eval:family` runs `scripts/eval-family-prompts.ts`: tests `buildCompositionSegment` against 7 family scenarios (solo, Under 2, 9–12, mixed, beach/city/adventure destinations) with 20 assertions. Exits with code 1 on failure. `npm run eval:recommendations` runs the restaurant recommendations eval. `npm run eval:alternatives` runs the 5-case restaurant alternative eval against `/api/itinerary/alternative`, scoring each replacement (different cuisines, budgets, destinations, vibes, company types) with Opus 4.6 as LLM-as-judge — requires the dev server running. `npm run eval:location` runs `scripts/eval-itinerary-location.ts`: tests `/api/itinerary/edit` location constraint with 5 trap cases (wrong-city items in context — e.g. Eiffel Tower in Amsterdam, Pergamon Museum in Lisbon), uses Claude Sonnet as LLM-as-judge to verify suggestions stay in the destination city. Requires dev server running. PHI-118 extracted every eval's logic into `lib/evals/<suite>/{cases.ts, runner.ts, judge.ts}` with shared scaffolding in `lib/evals/{judge.ts, site-auth.ts, types.ts}`; each `scripts/eval-X.ts` is now a thin wrapper (≤30 lines) calling `main()` from its suite. CLI output is byte-identical to pre-refactor.
...
│   ├── itinerary/                # Day-view itinerary — drag/drop, remove, AI swap/add, conflict banner, travel connectors; passes travelerCount/childrenAges to edit API
│   │   ├── page.tsx              # Page state/effects/handlers; ItineraryHeader / DayTimeline render the chrome
│   │   ├── *.tsx                 # ItineraryHeader, DayTimeline, DaySection (+ skeletons), ActivityCard, TripShapeBar, TravelConnectorRow, UndoToast, SuggestButton, AddSuggestionCard, BookingLinks
│   │   └── itinerary-{constants,helpers,types}.ts
...
│   ├── team/                     # Product agents — Kanban / Product team / PM / Coach tabs
│   │   ├── page.tsx              # TeamPage shell (tab switcher + Build/Research toggle)
│   │   ├── *.tsx                 # KanbanTab, ProductTeamTab, PMTab, ProductCoachTab, CardDetailPanel, PastConversations, shared.tsx (MarkdownText, AgentBubble, …)
│   │   └── team-{types,constants,data,download}.ts  # types · prompts/models · Supabase helpers · PRD download helpers
│   ├── guides/
...
│   │   ├── activities-stream/route.ts  # POST: streaming activity preview (onboarding step 5 — was step 4 pre-PHI-90)
...
│   │   ├── neighborhoods/route.ts      # PHI-100: POST destination → cached `destination_neighborhoods` lookup or Claude Haiku tool_use call returning 4–6 cards
│   │   ├── destination/popular-picks/route.ts # PHI-102: POST { destination, travelCompany, childrenAges, styleTags } → cached `popular_picks_cache` lookup or Claude Haiku tool_use call returning 5–8 picks with friction/fit/pro_tip notes
│   │   ├── itinerary/
...
│   │   ├── places/details/route.ts   # PHI-111: POST { placeId } → { placeId, lat, lng, neighborhood } via Google Places Details v1; gates on checkApiLimit("google") and logs places-details usage
...
│   │   │   ├── conversations/route.ts # GET/POST/PATCH/DELETE team_conversations (admin client — 0020 RLS lockdown)
│   │   │   ├── objectives/route.ts    # GET/POST/PATCH/DELETE kanban objectives (admin client)
│   │   │   ├── memory/route.ts        # GET/PUT Sarah's agent_memory row (admin client)
│   │   │   └── prd-feedback/route.ts  # GET/POST prd_feedback (admin client)
│   │   ├── evals/
...
│   ├── supabase-admin.ts         # PHI-61: server-only service-role client — the only path to system/admin tables
...
│   ├── neighborhood-gen-prompt.ts # PHI-100: Haiku system prompt + tool schema + cache key helper for /api/neighborhoods
│   ├── popular-picks-prompt.ts   # PHI-102: Haiku system prompt + tool schema + cache-key helpers (sortedStyleTags, sortedAgeBands) for /api/destination/popular-picks
│   ├── guides.ts                 # Client-safe shared types (Guide, Tip, Level) and helpers (getLevel, LEVEL_BADGE)
│   └── guides-server.ts          # Server-only guide/tip DB helpers (awardGuidePoints, incrementTipViews) on the admin client
├── scripts/
...
| Table | Purpose |
|---|---|
| `travelers` | Onboarding data — destination, dates, hotel, activities, account, traveler_count (int), children_ages (text[]), travel_company (text), style_tags (text[]), budget_tier (text), auth_user_id (uuid, FK → auth.users.id, PHI-59), is_primary (bool, default true, PHI-60), claimed_at (timestamptz, PHI-60), user_seeded_activities (text[], nullable, PHI-90 — traveller-typed must-dos captured at welcome step 4 and passed to /api/itinerary/generate as anchors; null/absent = existing prompt path runs unchanged), anchor_neighborhood (text, nullable, PHI-100 — soft area anchor selected via the welcome step-2 neighbourhood picker; only consulted by activity-gen / itinerary-gen when no hotel is set and the trip is single-leg; null/absent = existing prompt path runs unchanged), flex_month (text, nullable, PHI-99 — ISO month e.g. "2026-10" captured when the traveller chose "Not sure yet — I'm just exploring →" on welcome step 1; mutually exclusive with leg dates), flex_nights (integer, nullable, PHI-99 — duration paired with flex_month, 1–30), hotel_place_id (text, nullable, PHI-111 — Google Places ID of the hotel selected via PlacesAutocomplete on welcome step 2; mirrors legs[0].hotelPlaceId), hotel_lat (double precision, nullable, PHI-111), hotel_lng (double precision, nullable, PHI-111), hotel_neighborhood (text, nullable, PHI-111 — resolved neighbourhood label, e.g. "Tanjong Pagar"; the four hotel_* columns mirror legs[0]'s rich fields for single-leg-aware downstream consumers, and per-leg rich coords live inside legs JSONB for multi-leg). Name and email are nullable (collected at the account step, after row creation). `auth_user_id` is null until the user clicks the magic link; nullable so existing localStorage-only travellers keep working. PHI-60 (claim flow): a single `auth_user_id` may own multiple rows; `is_primary=true` marks the row the dashboard defaults to (claim flow ensures exactly one primary per user); `claimed_at` is stamped when the row is linked via the claim flow (null for legacy rows linked silently in PHI-59). |
...
| `popular_picks_cache` | PHI-102 cache for the welcome step-4 popular-picks affordance — city_key (lowercased), city_display, travel_company, children_age_bands (text[], sorted), top_style_tags_sorted (text[], sorted + lowercased — Luca's catch from refine to prevent reorder cache misses), picks (jsonb array of `{name, context_note, category: friction\|fit\|pro_tip}`), model. Unique index on (city_key, coalesce(travel_company,''), coalesce(children_age_bands,'{}'), coalesce(top_style_tags_sorted,'{}')) so per-profile shards never collide. First visitor for a (city × profile) shape triggers the Claude Haiku call; subsequent visitors with the same profile hit the cache. |
| `eval_suite_runs` | PHI-119 — one row per "I clicked Run" event on `/admin/evals/suites`. Columns: suite (slug), started_at, finished_at (null until done), status (`queued`/`running`/`succeeded`/`failed`/`cancelled`), model (null for offline suites), total_cost_usd, pass_rate (0-100), summary_score (0-100, mirrors pass_rate for offline suites with no LLM judge), notes (failure message excerpt), created_by (`"philip"` for now). Indexed on (suite, started_at desc) so the per-suite history query is a no-op. |
| `eval_case_runs` | PHI-119 — one row per case within a suite run. FK to `eval_suite_runs(id) on delete cascade`. Columns: case_name, run_index (0 for single-run suites; 0..N-1 for multi-run like anchors at 3×), programmatic_pass (bool, null for suites without programmatic checks), judge_score, judge_reasoning, output_snippet (capped at 1KB — full output regenerable from inputs), cost_usd, duration_ms, error (the failed-assertion summary string when programmatic_pass=false). Indexed on suite_run_id. |
...
**PHI-90 migration — user-seeded must-dos** (run once, idempotent; `db/migrations/0011_user_seeded_activities.sql`):
```sql
alter table travelers
...
**PHI-111 migration — hotel coordinates** (run once, idempotent; `db/migrations/0015_hotel_coordinates.sql`):
```sql
alter table travelers
...
```
All four columns are nullable / additive. Existing rows stay NULL — the legs JSONB `hotel` name string is still the visible hotel for those travellers, and any downstream consumer treats missing coords as "no hotel" (same fallback as the skip-hotel path). The trip schema (`lib/trip-schema.ts`) is the source of truth: `TripLeg` gained `hotelPlaceId`, `hotelLat`, `hotelLng`, `hotelNeighborhood` optional fields so multi-leg trips carry one coord set per leg. The route mirrors `legs[0]`'s rich fields onto the four flat columns via `flatHotelColumnsFromLegs` (single-leg-aware consumers can read `travelers.hotel_lat` directly without walking legs). `PlacesAutocomplete` gains an optional `onSelectRich` callback that fires after the user picks a suggestion — it routes through the new `app/api/places/details/route.ts` (which runs `checkApiLimit("google")` + `logApiUsage` with `apiType: "places-details"` at $0.017/req). On rate-limit-exceeded or any failure the rich callback is silently skipped: the hotel name still lands, only the coords are absent and the row stays null on those columns.

**PHI-105 — no migration** (prompt + eval only; depends on PHI-111's data). `/api/itinerary/generate` accepts `hotelPlaceId/Lat/Lng/Neighborhood` in the POST body (the welcome page's `buildRichHotelFields()` and `/itinerary` regenerate threads them automatically). When both lat/lng arrive AND the trip is single-leg, the route builds a `hotelContext` object and forwards it to `buildItineraryGenPrompt`, which appends a "Hotel context" block at the end of `buildUserSeededAnchorsSegment` — naming the hotel, neighbourhood, coords (rounded to 4 dp), and a walking-distance cue (10–15 min for couples / solo, 5–10 min for families with kids Under 2 / 2–4). The flag-bias rule from PHI-103 still wins: when the hotel sits in a generic business district, the model is instructed to flag hotel-relative anchors rather than fabricate. When the traveller has no hotel coords (skip-hotel path OR legacy pre-PHI-111 row), the segment is byte-identical to pre-PHI-105 — the prompt path stays unchanged.

**PHI-102 migration — popular picks cache** (run once, idempotent; `db/migrations/0016_popular_picks_cache.sql` — slot 0014 went to PHI-107, 0015 to PHI-111). Creates the `popular_picks_cache` table per the row description above, with a unique index on `(city_key, coalesce(travel_company,''), coalesce(children_age_bands,'{}'), coalesce(top_style_tags_sorted,'{}'))` so the per-profile sharding is total under SQL NULL semantics. Style tags + age bands are sorted + deduped + lowercased in `lib/popular-picks-prompt.ts` (`popularPicksSortedStyleTags`, `popularPicksSortedAgeBands`) before they're written, so a reordered Step 3 selection hits the same row. **Model: Sonnet 4.6** — the PRD's Haiku starting point failed `eval:popular-picks` at 3.72/4.0 (Haiku hallucinated ~25% of venue names: Tsukiji on Kyoto, "Mizuki Shikibu Museum", "Pastel de Nata de Belém"); Sonnet 4.6 passes cleanly at 4.06/5 with zero fixtures under the 4.0 floor. Per-call cost ~5× higher (`$0.001` → `$0.005`) but cache hit-rate covers >70% of expected production traffic per the PRD's own cost posture, so net impact is small.

**PHI-119 migration — eval suite run persistence** (run once, idempotent; `db/migrations/0017_eval_suite_runs.sql`). Creates `eval_suite_runs` and `eval_case_runs` per the row descriptions above, plus adds a nullable `suite_run_id uuid` column on `ai_logs` (additive — existing rows stay NULL; partial index keyed where `suite_run_id is not null` mirrors the `session_id` index from PHI-40). Indexed for the two read paths the GUI runs: per-suite history (`idx_suite_runs_suite_started`) and per-run case detail (`idx_case_runs_suite`). The `status` column is a free text string rather than an enum so the runner can grow new states (e.g. `partial`, `cancelled`) in cards 3–5 without a migration; the canonical state machine lives in the registry + page state, not the schema.

**0020 migration — RLS lockdown on all remaining public tables** (**already applied to the live project** via the Supabase MCP on 2026-07-15; `db/migrations/0020_rls_enable_system_tables.sql`). Fixes the `rls_disabled_in_public` ERRORs from the 2026-07-12 Supabase security advisor email: enables RLS with **no policies** on the 19 flagged tables (`guides`, `tips`, `tip_ratings`, `team_conversations`, `objectives`, `agent_memory`, `prd_feedback`, `profiles`, `user_feedback`, `activity_feedback`, `anonymous_sessions`, `api_usage`, `api_limits`, `eval_test_cases`, `eval_results`, `eval_suite_runs`, `eval_case_runs`, `destination_neighborhoods`, `popular_picks_cache`) plus `ai_logs` (0010 had only revoked SELECT — anon still held write grants), and revokes all anon/authenticated table grants as belt-and-braces. Service-role admin client bypasses RLS, so every existing route keeps working; the anon key is now denied on every table without an explicit policy. Code changes shipped in the same commit: guide/tip routes moved to `lib/guides-server.ts` + admin client, `/team` browser access moved behind `/api/team/*`, and the legacy anon client `lib/supabase.ts` was deleted. The advisor now reports these tables as `rls_enabled_no_policy` (INFO) — that's the intended deny-by-default state, not a problem.
...
- **The itinerary-gen prompt lives in `lib/itinerary-gen-prompt.ts`** (PHI-90). Both `app/api/itinerary/generate/route.ts` and `scripts/eval-itinerary-anchors.ts` work against the same builder; the route imports `buildItineraryGenPrompt` and the eval drives the route end-to-end. The helper also exports `buildUserSeededAnchorsSegment(userSeededActivities, destination, legs)` — modelled on `buildCompositionSegment` — that builds the structured anchor block injected when must-dos are present. The block carries the hard rules: anchors placed on exactly one day/block, `seededByUser: true` on the matching item, wrong-city anchors routed into top-level `placement_notes`, no silent drops. When the column / payload is null/empty the prompt is byte-identical to the pre-PHI-90 shape.
...
- `npm run eval:anchors` (PHI-90, PHI-94, PHI-95, PHI-105) — runs the 11-case user-seeded anchors eval against `/api/itinerary/generate`. Cases #1–#5 (PHI-90) cover the well-formed anchor traps: all-three-land, 10-must-dos-overflow, wrong-city-Louvre, splurge-dinner-block, family-90-min-tolerance. Cases #6–#8 (PHI-94) probe vague free-text entries — the shape real travellers actually paste from notes — under a resolve-OR-flag rubric: #6 Lisbon "that famous pastéis place" + "the viewpoint with the painted tiles"; #7 Tokyo "that ramen spot Anthony Bourdain went to"; #8 Lisbon "the famous viewpoint". Case #9 (PHI-95) is the Okafors honeymoon archetype Tokyo → Kyoto → Seoul with one anchor per leg (Sushi Saito → Tokyo, Fushimi Inari → Kyoto, DMZ tour → Seoul); a new `anchorsLandInExpectedLeg` programmatic helper asserts each placed anchor's `day.leg_index` matches the expected leg, hard-failing when the model silently routes an anchor onto the wrong leg (anchors surfaced only in `placement_notes` defer to `allAnchorsPlacedOrNoted` and don't trip the leg check). For vague entries, pass requires that each entry EITHER resolves to a real, recognisable in-destination venue (placed seededByUser=true, title NOT equal to the verbatim) OR is flagged in `placement_notes` quoting the verbatim and asking for a more specific name. Shipping the verbatim as a title = unhelpful-verbatim fail; inventing a fabricated venue = hallucination fail. Programmatic checks run first (anchor placed or surfaced in `placement_notes`; trip stays in destination; `seededByUser` flag set on matching items; wrong-city anchors filtered into `placement_notes`; splurge-dinner anchors land in `evening` block; vague anchors resolve or get flagged with framing; multi-leg anchors land on the expected `leg_index`). When programmatic checks pass, Sonnet 4.6 LLM-judge scores against a 3-4-criterion per-case rubric (the multi-leg case adds a leg-routing preamble and an items dump tagged with `[leg N]` / `[leg N, transition]`). Pass gate: every case passes (average judge score ≥7 AND every run passes programmatic checks). PHI-96: each case now runs 3× to absorb judge variance — the case score is the mean of 3 judge scores, and ANY single run failing programmatic checks fails the whole case. The 3× output shows per-run scores on a `Run 1: 9/10 · Run 2: 8/10 · Run 3: 7/10 → avg 8.0/10` sub-line under the existing PASS/FAIL header. Run before any prompt edit in `lib/itinerary-gen-prompt.ts`. Cost ~$2.20/run (11 cases × 3 runs; the 9-day multi-leg case lengthens the judge input). PHI-105 added two single-run cases #10 (resolve, Pullman Singapore Hill Street + "the noodle place near our hotel" — model must resolve to a real Tanjong Pagar / Clarke Quay walking-distance noodle shop AND surface the substitution in `placement_notes`) and #11 (flag, Crowne Plaza Changi Airport + same anchor — model must flag in `placement_notes` rather than fabricate). The `hotelRelativeAnchorResolves(verbatim)` and `hotelRelativeAnchorFlags(verbatim)` programmatic helpers gate before the judge runs.
- `npm run eval:popular-picks` (PHI-102) — runs the 18-case popular-picks quality eval against `/api/destination/popular-picks`. 6 cities (Lisbon / Tokyo / Kyoto / Bangkok / Málaga / New York) × 3 profile shapes (solo-female / family-under-5 / business-extender). Each fixture drives the live Haiku route, then Sonnet 4.6 LLM-judges the picks via `tool_use` against three criteria: factual accuracy (no hallucinated venues), profile-fit (notes / picks match THIS traveller), useful-friction (Elena's hard rule — context notes that could appear verbatim on the venue's own marketing page fail). Pass gate: average overall ≥4/5 AND no single fixture <3/5 — same per-fixture floor as `eval:country-destination` so uneven failure can't slip through. Cost ~$1.20/run (18 Haiku calls + 18 Sonnet judges, with subsequent runs reading from the `popular_picks_cache` row written on the first call). Run before any prompt edit in `lib/popular-picks-prompt.ts` or `app/api/destination/popular-picks/route.ts`.
- `npm run eval:alternatives` (PHI-118 wiring) — runs the 5-case restaurant alternative eval against `/api/itinerary/alternative`. Covers Italian / Japanese / Spanish / Portuguese / French dinner-swap scenarios across mid-range, budget, luxury budgets and couple / solo / family / friends company shapes. Opus 4.6 LLM-as-judge scores against 6 criteria per case (alternative is real and specific to the destination, different from the replacement, fits the time block, price tier matches budget, includes valid `booking_meta.search_query`, description references the destination). Pass gate: judge score ≥7/10 per case. The script existed in the repo pre-PHI-118 but was never wired into `package.json`; PHI-118 added the entry alongside extracting the suite into `lib/evals/alternatives/`. Run before any prompt edit in `app/api/itinerary/alternative/route.ts`. Cost ~$0.15/run.
- All evals above call the live Anthropic API and cost ~$0.15–$2.20 per run on Sonnet 4.6 / Opus 4.6. Keep `lib/api-costs.ts` rates current.
- **PHI-118 refactor (eval-runner extraction).** Every eval suite lives in `lib/evals/<suite>/`: `cases.ts` (test data + per-case programmatic check helpers), `runner.ts` (per-case `runOne` + the CLI `main` that prints + summarises + exits), and — when the suite has an LLM-as-judge step — `judge.ts` (rubric + judge call). Shared scaffolding lives in `lib/evals/judge.ts` (`runToolUseJudge<T>` for the Sonnet 4.6 + `tool_use` pattern that anchors / country-destination / popular-picks use; `runRawJudge` + `parseJsonJudgeResponse` for the raw-JSON + markdown-fence-stripping pattern that location / recommendations / alternatives use), `lib/evals/site-auth.ts` (one `bootstrapSiteAuth` shared by location / anchors / popular-picks), and `lib/evals/types.ts` (forward-looking `EvalCase` / `EvalCaseResult` / `SuiteRunner` shapes for the cards 2–5 GUI wiring). Each `scripts/eval-X.ts` is a ≤30-line wrapper calling the suite's `main()`. CLI output is byte-identical pre/post refactor — verified by capturing the offline subset (`eval:family`, `eval:free-form-detect`) before and after and diffing to zero.
...
- **PHI-111 — `onSelectRich` is the additive opt-in for rich Place data.** Existing callers continue passing only `onSelect: (description: string) => void` — that signature is locked. New callers that need lat/lng/neighborhood add `onSelectRich: (rich: PlaceRichSelection) => void` alongside it. The component always fires `onSelect` first; if `onSelectRich` is supplied it follows up with a `POST /api/places/details` (server-side rate-limit + cost-log) and invokes `onSelectRich` on success. Any failure or limit-hit silently skips the rich callback — the hotel name is captured, only the coords are absent. Always invalidate the captured rich payload on the next `onChange` (the user is now describing a different place).

### Supabase
- Never create a new client inline. Pick the right shared client: `getSupabaseAdminClient()` (`lib/supabase-admin.ts`) for system/admin tables, `getSupabaseServerClient()` for user-owned tables under RLS, `getSupabaseBrowserClient()` for client components needing the session. The legacy anon client (`lib/supabase.ts`) was deleted in the 0020 RLS lockdown.
- Use `.single()` when expecting one row; check the `error` field before using `data`.
- Points and tip views are incremented atomically via the `increment_guide_points` / `increment_tip_views` RPCs (`db/migrations/0019_points_rpc.sql`), wrapped by `awardGuidePoints()` / `incrementTipViews()` in `lib/guides-server.ts` (server-only, admin client; the client-safe types/constants stay in `lib/guides.ts`). Until that migration is applied, the helpers silently fall back to the legacy read-then-write pattern (racy but no worse than before) — apply 0019 in the Supabase dashboard to close the race.
...
| `rise_itinerary_placement_notes` | PHI-90: plain-text `placement_notes` string from the last `/api/itinerary/generate` response, surfaced as an amber callout above Day 1 on `/itinerary`. Cleared on a clean generate (no notes) so stale notes don't survive a Regenerate. PHI-114 narrowed scope to anchor surfacing only (mode-2 resolutions, mode-3 flags, wrong-city omissions, capacity overflow); travel facts live in `rise_itinerary_time_sensitive_alerts`. |
| `rise_itinerary_time_sensitive_alerts` | PHI-114: JSON-encoded `string[]` (max 4) of one-sentence travel facts the traveller must verify or act on — closures, pre-booking requirements, seasonal cutoffs, peak-time advice, transport quirks. Surfaced as a "Before you go" amber block ABOVE the `placement_notes` callout on both the welcome preview and `/itinerary`. Cleared on a clean generate (no alerts) and wiped alongside `rise_itinerary` when the cached itinerary belongs to a previous trip. |
...
- Use `getSupabaseServerClient()` from `lib/supabase-server.ts` in Server Components / Route Handlers when you need `auth.uid()`; use `getSupabaseBrowserClient()` from `lib/supabase-browser.ts` in client components. System/admin tables are reached only through `getSupabaseAdminClient()` — since the 0020 RLS lockdown every public table has RLS enabled and the anon key is denied everywhere it has no policy.
