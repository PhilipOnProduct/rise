...
| Table | Purpose |
|---|---|
| `travelers` | Onboarding data — destination, dates, hotel, activities, account, traveler_count (int), children_ages (text[]), travel_company (text), style_tags (text[]), budget_tier (text), auth_user_id (uuid, FK → auth.users.id, PHI-59), is_primary (bool, default true, PHI-60), claimed_at (timestamptz, PHI-60), user_seeded_activities (text[], nullable, PHI-90 — traveller-typed must-dos captured at welcome step 4 and passed to /api/itinerary/generate as anchors; null/absent = existing prompt path runs unchanged). Name and email are nullable (collected at the account step, after row creation). `auth_user_id` is null until the user clicks the magic link; nullable so existing localStorage-only travellers keep working. PHI-60 (claim flow): a single `auth_user_id` may own multiple rows; `is_primary=true` marks the row the dashboard defaults to (claim flow ensures exactly one primary per user); `claimed_at` is stamped when the row is linked via the claim flow (null for legacy rows linked silently in PHI-59). |
...
- `npm run eval:anchors` (PHI-90, PHI-94) — runs the 8-case user-seeded anchors eval against `/api/itinerary/generate`. Cases #1–#5 (PHI-90) cover the well-formed anchor traps: all-three-land, 10-must-dos-overflow, wrong-city-Louvre, splurge-dinner-block, family-90-min-tolerance. Cases #6–#8 (PHI-94) probe vague free-text entries — the shape real travellers actually paste from notes — under a resolve-OR-flag rubric: #6 Lisbon "that famous pastéis place" + "the viewpoint with the painted tiles"; #7 Tokyo "that ramen spot Anthony Bourdain went to"; #8 Lisbon "the famous viewpoint". For vague entries, pass requires that each entry EITHER resolves to a real, recognisable in-destination venue (placed seededByUser=true, title NOT equal to the verbatim) OR is flagged in `placement_notes` quoting the verbatim and asking for a more specific name. Shipping the verbatim as a title = unhelpful-verbatim fail; inventing a fabricated venue = hallucination fail. Programmatic checks run first (anchor placed or surfaced in `placement_notes`; trip stays in destination; `seededByUser` flag set on matching items; wrong-city anchors filtered into `placement_notes`; splurge-dinner anchors land in `evening` block; vague anchors resolve or get flagged with framing). When programmatic checks pass, Sonnet 4.6 LLM-judge scores against a 3-4-criterion per-case rubric. Pass gate: every case passes (score ≥7 AND no programmatic failure). Run before any prompt edit in `lib/itinerary-gen-prompt.ts`. Cost ~$0.50/run.
...
| Key | Contents |
|---|---|
| `rise_traveler` | Full traveller object (name, email, destination, dates, hotel, travelCompany, travelerCount, childrenAges, travelerTypes, budgetTier, activities, **userSeededActivities** — PHI-90 must-dos captured at welcome step 4, optional `string[]`, omitted when the user skipped the step) |
