...
- `npm run eval:anchors` (PHI-90) — runs the 5-case user-seeded anchors eval against `/api/itinerary/generate`. Programmatic checks run first (anchor placed or surfaced in `placement_notes`; trip stays in destination; `seededByUser` flag set on matching items; wrong-city anchors filtered into `placement_notes`; splurge-dinner anchors land in `evening` block). When programmatic checks pass, Sonnet 4.6 LLM-judge scores against a 3-4-criterion per-case rubric. Pass gate: every case passes (score ≥7 AND no programmatic failure). Run before any prompt edit in `lib/itinerary-gen-prompt.ts`. Cost ~$0.30/run.
