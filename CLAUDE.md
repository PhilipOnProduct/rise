...
- **On close** — after `npm run build` passes cleanly **and the change has been walked on the code path the diff actually runs on**, `save_comment` with What changed / What was tested / What's deferred, then move the card to **Done** via `save_issue`. The walk starts from the user's actual entry point (usually `/`, not the route touched).
